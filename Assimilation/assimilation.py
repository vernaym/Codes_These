...
class RandomSampling(Assimilation):
    ...
    def dynamic_error_estimation(self, parameters, obs_auto, var='mu', delta=0.1):
        ...
        #obs_auto = obs_auto[obs_auto["rr"] >= 1]  # No precipitation in reference ==> keep ANTILOPE (gauge obstructed ?) --> solve missed precipitation over ridges. (ex : Savoie 20220110)
...
#                    ratio_arome[mask] = 1  # Do not introduce precipitation on pixel with no precipitation and no reference
            ...
            ratios.append(rat)
            error = np.abs(rat - 1) * ((parameters[var].data + delta2) / rat - delta2)
            errors.append(error)
...
#                self.plot_array(D/Wm, parameters, 'D/W', f'{self.date_str}/D_over_W_{self.domain}.pdf', cmap=plt.cm.viridis, vmin=0, vmax=0.2, domain=self.domain)
...
#                self.plot_array(D/2, parameters, 'D/2', f'{self.date_str}/D_over_2_{self.domain}.pdf', cmap=plt.cm.viridis, vmin=0, domain=self.domain)
            pass
