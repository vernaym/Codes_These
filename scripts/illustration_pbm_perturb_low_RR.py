...
def plot_var(ds, xpid, date=None, mask=True):

    if mask:
        # mask glacier/forest covered pixels
        ds = ct.maskgf(ds)

    tmp = (ds['Rainf'] + ds['Snowf']) * 3600.
    tmp = tmp.where(
        (tmp.time > pd.to_datetime('2021113006', format='%Y%m%d%H')) &
        (tmp.time <= pd.to_datetime('2021120106', format='%Y%m%d%H')), drop=True).sum(dim='time')
    tmp = tmp.where(tmp > 0)
    ...
    fig = plot2D.plot_ensemble(tmp, vmin=0, vmax=1, cmap=plt.cm.YlGnBu)
    savename = f'RR_ensemble_{xpid}.pdf'
...
if __name__ == '__main__':
    ...
    for xpid in xpids:
        ...
        plot_var(ds, shortid, date=date, mask=mask)
